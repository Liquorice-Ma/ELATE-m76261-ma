...
def _scan_problems():
    """Scan traffic matrix directories for available problems."""
    for topo_file in os.listdir(TOPOLOGIES_DIR):
        if not topo_file.endswith(".json"):
            continue
        topo_fname = os.path.join(TOPOLOGIES_DIR, topo_file)
        for model in TM_MODELS:
            for tm_fname in iglob(
                "{}/{}/{}*_traffic-matrix.pkl".format(
                    TM_DIR, model, topo_file)
            ):
                vals = os.path.basename(tm_fname)[:-4].split("_")
                _, traffic_seed, scale_factor = vals[1], int(vals[2]),\
                    float(vals[3])
                GROUPED_BY_PROBLEMS[
                    (topo_file, model, scale_factor)].append(
                    (topo_fname, tm_fname))
                PROBLEMS.append((topo_file, topo_fname, tm_fname))
            for tm_fname in iglob(
                ...
                _, traffic_seed, scale_factor = vals[1], int(vals[2]),\
                    float(vals[3])
                GROUPED_BY_HOLDOUT_PROBLEMS[
                    (topo_file, model, scale_factor)].append(
                    (topo_fname, tm_fname))
                HOLDOUT_PROBLEMS.append((topo_file, topo_fname, tm_fname))
...
def get_problems(args):
    """Get problem list from args."""
    if (args.topo, args.tm_model, args.scale_factor) not in GROUPED_BY_PROBLEMS:
        raise Exception(
            f'Traffic matrices not found for topo={args.topo}, '
            f'tm_model={args.tm_model}, scale={args.scale_factor}')
    problems = []
...
def get_args_and_problems(formatted_fname_template, additional_args=[]):
    ...
    parser.add_argument(
        ...
        help="network topology JSON file (in topologies/)")
    parser.add_argument(
    ...
    # ELATE mode
    parser.add_argument(
        '--leo', action='store_true', default=False,
        help='enable ELATE mode (elephant/mice separation + MARL)')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='number of ADMM steps (TEAL mode only)')
    parser.add_argument(
    ...
    # In LEO mode, force MLU objective
    if args.leo:
        args.obj = 'min_max_link_util'
